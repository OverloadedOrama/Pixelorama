# From https://docs.godotengine.org/en/stable/tutorials/plugins/editor/visual_shader_plugins.html
# perlin_noise_3d.gd
@tool
...
func _get_code(input_vars: Array[String], output_vars: Array[String], _mode: Shader.Mode, _type: VisualShader.Type) -> String:
	...
	if uv.is_empty():
		uv = "UV"
	var offset := input_vars[1]
	if offset.is_empty():
		offset = "vec3(0.0)"
	var scale := input_vars[2]
	if scale.is_empty():
		scale = str(get_input_port_default_value(2))

...
func _init() -> void:
	...
	set_input_port_default_value(2, Color.BLACK)
	set_input_port_default_value(3, Color.WHITE)
...
func _get_code(input_vars, output_vars, mode, type):
	...
	if input_vars[0]:
		uv = input_vars[0]
	var color_1 := "vec3(0.0, 0.0, 0.0)"
	var color_2 := "vec3(1.0, 1.0, 1.0)"
	if not input_vars[2].is_empty():
		color_1 = input_vars[2]
	if not input_vars[3].is_empty():
		color_2 = input_vars[3]

	return "%s.xyz = checkerboard(%s.xy, %s.xy, %s, %s);" % [output_vars[0], uv, input_vars[1], color_1, color_2]
